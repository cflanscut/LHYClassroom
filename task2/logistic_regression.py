...
def function(x, w, b):
    function_output = sigmoid(np.matmul(x, w) + b)
    y = np.empty([len(function_output), 1])
    for i in range(len(y)):
        y[i, 0] = function_output[i]
    return y
...
def gradient(x, y_label, w, b):
    ...
    w_gradient = -np.sum(
        (y_label - y_predicition).transpose() * x.T,
        1)  # 这里用的是*号，即y是1*N的相量，与x.T(k*N维度)的每一行相乘，然后得到k*N的结果，再对每一列求和
    b_gradient = -np.sum((y_label - y_predicition))
